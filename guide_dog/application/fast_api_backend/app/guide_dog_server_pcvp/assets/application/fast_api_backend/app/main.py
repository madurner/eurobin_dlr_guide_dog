...
async def detect_stuff(camera: Camera, image: cv2.Mat, guide_dog_server: GuideDogServerPCVP) -> dict:
    ...
    resp = False
    while resp is False and success is True:
...
async def start_pipeline(key: Annotated[str, Depends(handle_api_key)], pcvp_modules: Annotated[dict, Body] = None):
    # Extract pcvp_modules from request body, defaulting to empty list
    modules = pcvp_modules.get("pcvp_modules", []) if pcvp_modules else []
    try:
        await create_and_start_pipeline(key, pcvp_modules=modules, timeout=60.0)
    except TimeoutError as e:
        raise HTTPException(
            status_code=404,
            detail={
                "message": "Pipeline startup failed: timeout waiting for LN Manager",
                "error": str(e),
                "state": "error",
            },
        ) from e
        ...
        raise HTTPException(
            status_code=404,
            detail={
                "message": "Pipeline startup failed: timeout during initialization",
                "error": str(e),
                "state": "error",
            },
        ) from e
...
async def get_pipeline_status(ln_manager_runner: Annotated[LNRunner, Depends(get_ln_manager_runner)]):
    try:
        status_ok = ln_manager_runner.check_all_processes_ready()
        if status_ok:
            status = "ready"
        else:
            status = "stopped"
    except (BrokenPipeError, OSError, AttributeError):
        # BrokenPipeError occurs when trying to communicate with stopped process
        # OSError can occur when process stdin is closed
        # AttributeError can occur when ln_manager_process is None
        status = "stopped"
    return {
...
async def stop_pipeline(
    key: Annotated[str, Depends(handle_api_key)], ln_manager_runner: Annotated[LNRunner, Depends(get_ln_manager_runner)]
):
    await ln_manager_runner.stop_all()
